...
const CurrencySelector = props => {
  ...
  return (
    <div className="currency-selector" ref={dropdownRef}>
      <button className="dropdown-button" onClick={onButtonClick}>{selectedCurrency}</button>
      {isDropdownOpen && (
        <div className="dropdown-wrapper">
          {currencyOptions.map(currency => {
            return (
              <div className="dropdown-item" onClick={onOptionClick(currency)} key={currency}>{currency}</div>
            );
          })}
        </div>
      )}
